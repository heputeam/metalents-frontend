...
const ButtonGroup: React.FC<IButtonGroupProps> = ({ status }) => {
  ...
  const UploadFilesBtn = () => (
    <Button
      variant="contained"
      classes={{
        root: classNames(styles['button'], styles['upload-files-button']),
      }}
    >
      Upload files
    </Button>
  ...
  const ReplyToCancellationBtn = () => (
    <Button
      variant="contained"
      classes={{
        root: classNames(
          styles['button'],
          styles['reply-to-cancelltaion-files-button'],
        ),
      }}
    >
  ...
  switch (status) {
    case EOrderStatus.InProgress:
      return (
        <Stack direction="row" spacing={10}>
          <SendMessageBtn />
          <UploadFilesBtn />
          {(Math.floor(Date.now() / 60000) % 60) % 2 === 0 && <CancelBtn />}
        </Stack>
      );
    ...
    case EOrderStatus.Refund:
      return (
        <Stack direction="row" spacing={10}>
          <SendMessageBtn />
          {(Math.floor(Date.now() / 60000) % 60) % 2 === 1 ? (
            <CancelingBtn />
          ) : (
            <ReplyToCancellationBtn />
          )}
        </Stack>
      );
