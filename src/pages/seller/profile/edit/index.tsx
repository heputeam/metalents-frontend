...
import { postUserUpdate } from '@/service/user';
...
export interface IEditSellerProfileProps {}
...
const EditSellerProfile: React.FC<IEditSellerProfileProps> = ({}) => {
  ...
  >(
    ...
    },
    {
      ...
      onSuccess: (resp) => {
        ...
        setTimeout(() => {
          history.push('/seller/profile?sidebar');
        }, 1000);
      },
      onError: (err) => {
  ...
  const validationSchema = yup.object({
    ...
    shopname: yup
      ...
      .test(
        ...
        async (shopname?: string) => {
          if ((userInfo && shopname === userInfo.shopname) || !shopname) {
            return true;
          }

          const res: any = await getShopnameCheck(shopname?.trim());
    ...
    company: yup
      .string()
      .max(50, 'Allow only no more than 50 characters')
      .test('notEmptyCompany', 'Please enter your company', (career) => {
        if (!career) {
          return false;
        }
